...
import { Plus, Trash2, Search, Filter, X, Check } from 'lucide-react'
...
const empty = { name: '', type: '', colors: [], weatherTags: [], occasions: [] }

export default function Wardrobe({ wardrobe, addItem, removeItem }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        {filtered.map(item => (
          <WardrobeItem key={item.id} item={item} onRemove={() => removeItem(item.id)} />
        ))}
      </div>
    </div>
  )
